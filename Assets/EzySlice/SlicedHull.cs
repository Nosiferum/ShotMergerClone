...
namespace EzySlice {
    ...
    public sealed class SlicedHull {
        ...
        public GameObject CreateUpperHull(GameObject original, Material crossSectionMat) {
            ...
            if (newObject != null) {
                newObject.transform.localPosition = original.transform.position; //changed from local to global
                newObject.transform.localRotation = original.transform.rotation; //changed from local to global
